...
shared_context "rake" do
  ...
  let(:task_path) { "lib/tasks/databases" }
  subject         { rake[task_name] }

  def loaded_files_excluding_current_rake_file
    $".reject {|file| file == File.expand_path("#{task_path}.rake", MIGRATOR_ROOT) }
  end

  before do
    Rake.application = rake
    Rake.application.rake_require(task_path, [MIGRATOR_ROOT], loaded_files_excluding_current_rake_file)
